...
serving web and mobile platforms (React-Native), serving as the single source of truth for UI/UX across all
consumer-facing digital products - the main eShop website, and mobile applications.
...
**Scale:** DLS underpins all Singtel consumer-facing digital products - spanning millions of users across Singapore's
...
- Consistency: Achieved visual and UX consistency across 4 Singtel consumer-facing digital products through a single
  versioned library. UX team was able to effect change in a consistent manner across all products and the process was
  streamlined.
- Velocity: Product teams report faster feature delivery by consuming DLS components rather than building from scratch -
  estimated 30-40% reduction in man hours allocated to frontend time work
...
- Engineer experience: Storybook documentation and TypeScript typings reduced the "how do I use this?" questions
  directed at the DLS team, freeing capacity for new component development. Product teams can focus on their core
  business logic and not worry about the UI.
...
flowchart TD
    subgraph UX team["DLS UX Team"]
        figma[DLS Figma]
    end
    subgraph DLS eng["DLS Engineering Team"]
        web[DLS Web]
        mobile[DLS Mobile]
        web-comp[DLS Web Components]
        rn-comp[DLS React Native Components]
        storybook[DLS Web Storybook]
        preview[DLS React Native Preview App]
    end
...
flowchart TB
  A[Pull Request] --> B["`Lint + Unit Tests + Storybook Build + Visual Regression Tests`"]
  B --> C[Merge to main]
  C --> D["`Build: tokens → web → native`"]
  D --> E["`Publish to private npm registry`"]
  E --> F["`Automated Storybook deployment`"]
```
...
- CSS-in-JS solution: DLS uses CSS-in-JS solution (styled-components) which adds some runtime overhead . But we decided
  to swallow this cost in favour or developer experience.
...
### Challenge 1: Breaking Change coordination
...
## Ownership & Leadership activities
...
- Led quarterly DLS community of practice sessions - open Teams channel for product team engineers to surface friction,
  request new components, and understand upcoming changes
