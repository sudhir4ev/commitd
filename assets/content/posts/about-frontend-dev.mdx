...
- Measurement comes before optimization. I instrument systems before making improvements. Tools like Lighthouse, Web
  Vitals, bundle analysis (webpack-bundle-analyzer, source-map-explorer), and real-user monitoring form the basis of my
  process, rather than relying on intuition.
...
With backend teams, I drive contract-first API design: agreeing on schema, error shapes, and pagination before a line of
code is written. (Pact, OpenApi) This avoids the all-too-common situation where the frontend is blocked by an API that
doesn't match what the UI actually needs.
